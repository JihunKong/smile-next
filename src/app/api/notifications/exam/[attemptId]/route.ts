...
import { sendExamResultNotification } from '@/lib/services/examNotificationService'

/**
 * POST: Send exam result notification for a specific attempt
 */
export async function POST(
  ...
  try {
    ...
    const result = await sendExamResultNotification({
      attemptId,
      recipientEmail,
      includeOwner,
    })
    ...
    console.error('[POST /api/notifications/exam/[attemptId]] Error:', error)
    return NextResponse.json(
      { error: 'Failed to send exam notification' },
      { status: 500 }
    )
  }
}
